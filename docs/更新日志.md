...
## v1.11.0
- 重构后端为 `/internal` 分层：通用模型接口集中在 `internal/core/types.go`，数据源适配器迁移至 `internal/adapters/`，数据库管理器与 SQLite 配置整理至 `internal/infra/` 并导出 `DBManager`、`sqlitecfg` 等组件，`main.go` 仅负责装配配置、日志与 HTTP 服务。
- 新建 `internal/api/` 模块承载 Gin 路由、查询缓存与分页归并逻辑，引入 `panicRecoveryMiddleware`、结构化 CORS/静态资源注册以及 `query_params.go` 中的参数校验工具，确保接口 panic 被捕获且错误上下文可追踪。
- Calibre/Legacy 适配器（`internal/adapters/*.go`）启用结构化 SQL 日志、统一的 `fmt.Errorf("xxx: %w", err)` 错误包装与 SQLite FTS 批量构建，新增本地热数据缓存以避免重复查询，并在此文档记录版本变更。

## v1.10.0
- 搜索接口（`main.go`）取消服务端二次分页，直接透传前端的 `LIMIT/OFFSET` 给各数据源并通过新建的堆归并逻辑保持 `id` 倒序结果，再将结果写入本地缓存（`search_cache.go`）以 5 分钟 TTL 存储“查询 + 页码”组合。
- Legacy/Calibre 适配器（`database/legacy_adapter.go`, `database/calibre_adapter.go`）在初始化阶段按需开启 WAL，并在重建 FTS5 索引时启用事务批量导入；Calibre 构造函数还统一采用 `JOIN calibre_books_fts` 的 MATCH 条件以减少 EXISTS 子查询。
- 高级查询构建器与 FTS 工具（`search/builder.go`, `search/fts.go`）将模糊检索改为基于 FTS5 JOIN 的 `f MATCH ?` 语法，并强制按照 `b.id DESC` 排序，单元测试（`search/builder_test.go`）同步更新预期。

## v1.9.0
- 搜索结果列表（`frontend/src/components/ResultsList.tsx`）新增网格/列表视图切换能力，并向下传递封面开关供不同视图共用；列表视图由新建的 `frontend/src/components/BookListItem.tsx` 渲染，保留紧凑横条式排版。
- 网格卡片组件（`frontend/src/components/BookItem.tsx`）与新建的列表条目组件都支持根据 `showCovers` 状态隐藏封面区域，避免单个组件过度复杂。

## v1.8.0
- Legacy 适配器（`database/legacy_adapter.go`）在初始化阶段自动构建 `books_fts` 虚拟表并同步 `books` 表数据，改用 MATCH 语法加速标题、作者等字段的模糊检索；Calibre 适配器（`database/calibre_adapter.go`）同步生成 `calibre_books_fts` 索引以支撑标题/作者/标签的全文搜索。
- 搜索构建器（`search/builder.go`、`search/fts.go`）新增 MATCH 查询语法与 FTS 查询字符串格式化工具，同时扩展单元测试覆盖（`search/builder_test.go`、`search/fts_test.go`）。

## v1.7.0
- 将 SQLite 驱动由 CGo 版的 `github.com/mattn/go-sqlite3` 替换为纯 Go 的 `modernc.org/sqlite`，同步更新 `database/calibre_adapter.go`、`database/legacy_adapter.go`、`internal/tools/deps.go` 以及 `go.mod`，并将 `sql.Open` 的驱动名切换为 `sqlite`，以便在无 C 编译环境的部署中也能正常运行。
